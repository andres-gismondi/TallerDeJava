...
import app.user.controller.http.request.*;
...
public class UserController {
    ...
    public ResponseEntity<String> setCategories(@RequestBody HttpRequestUserCategories httpRequestUserCategories, @RequestHeader("Authorization") String token) {
        String body = userService.setCategories(httpRequestUserCategories.getCategories(), httpRequestUserCategories.getUser(), token);
        return UtilsController.getResponseByString(body);
    }

    @RequestMapping(value = "/login", method = RequestMethod.POST)
    public ResponseEntity<HttpResponseUserLogin> login(@RequestParam("userName") String userName, @RequestParam("password") String password) {
        HttpHeaders responseHeader = new HttpHeaders();

        ResponseUserHeader responseBody = userService.loginUser(userName, password, responseHeader);
        if (!responseBody.getHeaders().get("Authorization").isEmpty()) {
            return new ResponseEntity<>(responseBody.getHttpResponseUserLogin(), responseBody.getHeaders(), HttpStatus.CREATED);
        }
        return new ResponseEntity<>(responseBody.getHttpResponseUserLogin(), HttpStatus.CONFLICT);
    }
    ...
    public ResponseEntity<String> createBillboard(@RequestBody HttpRequestUserBillboards httpRequestUserBillboards, @RequestHeader("Authorization") String token) {
        String body = userService.createBillboard(httpRequestUserBillboards.getBillboard(), httpRequestUserBillboards.getUser(), token);
        return UtilsController.getResponseByString(body);
    }
